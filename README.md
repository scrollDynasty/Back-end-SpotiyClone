Вот обновленный текст с добавлением якорей для навигации по разделам:
...
- [Dotenv](https://www.npmjs.com/package/dotenv) — утилита для загрузки переменных окружения из файла `.env`.
...
### Версия 3.5
- **👑 Роли пользователей**: 
  - При регистрации необходимо указать роль: `user`, `artist` или `admin`.
  - Возможность изменения роли пользователя по email (временно).
- **📊 Аналитика**: Базовая аналитика прослушиваний.
- **🛡️ Защиты от API-атак**: Добавлены новые механизмы защиты, такие как Helmet, XSS-Clean, Rate Limiting и другие.
