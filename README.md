...
В проекте реализован полный функционал восстановления пароля через электронную почту:
...
1. **Запрос на восстановление пароля**:
   ...
   - Сервер отправляет email с ссылкой для сброса пароля на указанный адрес.

2. **Сброс пароля**:
   - Пользователь получает письмо и переходит по ссылке для сброса пароля.
   - Фронтенд отображает форму для ввода нового пароля и отправляет запрос на `/auth/reset-password` с токеном и новым паролем.
...
- Электронные письма отправляются с использованием библиотеки `nodemailer`.
- Письма имеют красивый HTML-шаблон с кнопкой для сброса пароля.
